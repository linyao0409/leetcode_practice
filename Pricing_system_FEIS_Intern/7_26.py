...
def search_button_clicked():
    ...
    if search_input in df["ISIN"].values:
        ...
        esun_cb = tk.Checkbutton(new_window,text="玉山",variable=esun_var)
        esun_cb.pack()

        global sbc_var 
        sbc_var = tk.BooleanVar()
        sbc_var.set(int(row["SBC"]==1))
        sbc_cb = tk.Checkbutton(new_window,text ="星展",variable=sbc_var)
        sbc_cb.pack()

        global cathay_var 
        cathay_var = tk.BooleanVar()
        cathay_var.set(int(row["CATHAY"]==1))
        cathay_cb = tk.Checkbutton(new_window,text="國泰",variable=cathay_var)
        cathay_cb.pack()

        global fubon_var 
        fubon_var = tk.BooleanVar()
        fubon_var.set(int(row["FUBON"]==1))
        fubon_cb = tk.Checkbutton(new_window,text="富邦",variable=fubon_var)
        fubon_cb.pack()
        ...
        update_button = tk.Button(new_window, text="更新ISIN對應窗口", font=("微軟正黑體", 16))
        update_button.pack()
        ...
        add_button = tk.Button(new_window, text="新增ISIN-code", font=("微軟正黑體", 16))
