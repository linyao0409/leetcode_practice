...
def update_company(index,esun_var,sbc_var,cathay_var,fubon_var):
    df.iloc[index,1:] = [int(esun_var.get()),int(sbc_var.get()),int(cathay_var.get()),int(fubon_var.get())]

    output_file = 'ISIN_code_test.xlsx' # 覆蓋掉原先ISIN_code_test.clsx檔案
    df.to_excel(output_file, index=False)# 將DataFrame輸出成Excel檔案

def insert_new_isin(isin,esun_var,sbc_var,cathay_var,fubon_var):
    global df
    # 新的資料
    new_data = {
        'ISIN': isin, 
...
def search_button_clicked():
    ...
    if search_input in df["ISIN"].values:
        ...
        global sbc_var 
        sbc_var = tk.BooleanVar()
        sbc_var.set(int(row["SBC"]==1))
        sbc_cb = tk.Checkbutton(new_window,text ="星展",variable=sbc_var,font=checkbox_font)
        sbc_cb.pack()
        ...
        global fubon_var 
        fubon_var = tk.BooleanVar()
        fubon_var.set(int(row["FUBON"]==1))
        fubon_cb = tk.Checkbutton(new_window,text="富邦",variable=fubon_var,font=checkbox_font)
        fubon_cb.pack()
        ...
        update_button = tk.Button(new_window, text="更新ISIN對應窗口", font=("微軟正黑體", 16),command=lambda: update_company(index,esun_var,sbc_var,cathay_var,fubon_var))
        update_button.pack()
        ...
        sbc_checkbox = tk.Checkbutton(new_window, text='星展', variable=sbc_var, font=("微軟正黑體", 16))
        catchy_checkbox = tk.Checkbutton(new_window, text='國泰', variable=cathay_var, font=("微軟正黑體", 16))
        fubon_checkbox = tk.Checkbutton(new_window, text='富邦', variable=fubon_var, font=("微軟正黑體", 16))
        ...
        #def insert_new_isin(isin,esun_var,sbc_var,cathay_var,fubon_var):
        add_button = tk.Button(new_window, text="新增ISIN-code", font=("微軟正黑體", 16),command= lambda: insert_new_isin(search_input,esun_var,sbc_var,cathay_var,fubon_var))
...
Sbc_var_p3 = tk.BooleanVar()
Cathay_var_p3 = tk.BooleanVar()
Fubon_var_p3 = tk.BooleanVar()
...
Sbc_cb_p3 = tk.Checkbutton(page3, text='星展銀行', variable=Sbc_var_p3, font=checkbox_font)
Cathay_cb_p3 = tk.Checkbutton(page3, text='國泰銀行', variable=Cathay_var_p3, font=checkbox_font)
Fubon_cb_p3 = tk.Checkbutton(page3, text='富邦銀行', variable=Fubon_var_p3, font=checkbox_font)
...
bank_Sbc_checkbox = tk.Checkbutton(page4, text='星展銀行', variable=Sbc_var_p3, font=checkbox_font)
bank_Catchy_checkbox = tk.Checkbutton(page4, text='國泰銀行', variable=Cathay_var_p3, font=checkbox_font)
bank_Fubon_checkbox = tk.Checkbutton(page4, text='富邦銀行', variable=Fubon_var_p3, font=checkbox_font)
